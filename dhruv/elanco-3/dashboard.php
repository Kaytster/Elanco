...
<style>
	...
	.circular-progress::after {
		...
		left: 0;
		width: 100%;
		height: 0%; /* Initial height is 0%, will be animated with JS */
		background: linear-gradient(to top, var(--progress-color) 0%, var(--progress-color-light) 100%);
		border-radius: 0; /* Changed from 50% to create a filling effect from bottom */
		transition: height 1.5s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.3s ease;
		z-index: 1;
		opacity: 0.9; /* Increased opacity for more vibrant colors */
...
<script>
// Enhanced animation for all visual elements
document.addEventListener('DOMContentLoaded', function() {
    ...
    circles.forEach(circle => {
        ...
        // Set the fill height directly with inline style to ensure it works
        circle.style.setProperty('--progress-fill', `${value}%`);
        circle.style.setProperty('--fill-opacity', '0.9');
        circle.classList.add('fill-visible');
        
        // Apply shadow effect with RGB values
        if (rgbColor) {
            circle.style.boxShadow = `0 0 15px rgba(${rgbColor.r}, ${rgbColor.g}, ${rgbColor.b}, 0.4)`;
            circle.style.setProperty('--rgb-color', `${rgbColor.r}, ${rgbColor.g}, ${rgbColor.b}`);
        }
        
        // Add pulse animation for higher values
        if (value > 30) {
            let pulseSize = value;
            let growing = true;
            ...
            const pulse = setInterval(() => {
                ...
                circle.style.setProperty('--progress-fill', `${pulseSize}%`);
                
                // Update the glow intensity based on the pulse
                const glowIntensity = 0.4 + ((pulseSize - value) / 20);
                if (rgbColor) {
                    circle.style.boxShadow = `0 0 20px rgba(${rgbColor.r}, ${rgbColor.g}, ${rgbColor.b}, ${glowIntensity})`;
                }
            }, 50);
